...
Lampu::Lampu(float x, float y, float z, float rotation){
    ...
    glPushMatrix();
    	...
    	glPushMatrix();
    		...
    		glRotated(270, 1, 0, 0);
    		gluCylinder(pObj, 0.5, 0.5, 18, 25, 25);
    		glTranslatef(0,0,18);
    		glutSolidCube(1);
    		glTranslatef(0,1,0);
    		glutSolidCube(1);
...
Coin::Coin(float x, float y, float z, float s){
    ...
    glTranslatef(getX(),getY(), getZ());
        glPushMatrix();
            glColor4f(0.0f, 1.0f, 1.0f, 1.0f); //warna light blue :)
            glutSolidTeapot(getSize()/2);
        glPopMatrix();
    glPopMatrix();
}
